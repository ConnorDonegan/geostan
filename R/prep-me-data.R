...
prep_me_data <- function(ME, family, x) { # for x pass in x.list$x
  ...
  if (is.null(ME$ME)) {
     ...
     ## now deal with observational error in covariates, if any
        ...
        if  (!all(names(ME$ME) %in% names(x.df))) stop("All column names in ME$ME must be found in the model matrix (from model.matrix(formula, data)). This error may occur if you've included some kind of data transformation in your model formula, such as a logarithm or polynomial, which is not supported for variables with sampling/measurement error.")
