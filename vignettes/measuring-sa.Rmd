---
title: "Measuring and visualizaing spatial autocorrelation with **geostan**"
...
header-includes:
   - \usepackage{amsmath}
vignette: >
  %\VignetteIndexEntry{Measuring and visualizaing spatial autocorrelation with **geostan**}
...
The map shows that the contrast between the greater Atlanta metropolitan area and the rural counties is the predominant spatial pattern. Because the population is highly concentrated around metropolitan Atlanta, it helps to compare the mean of the variable (which is just 18%):
...
The Moran plot is a visualization of the degree of spatial autocorrelation (SA): on the horizontal axis are the `college` estimates while the vertical axis represents the mean neighboring value. The Moran coefficient, an index of SA, is printed at the top (MC=0.42) [@chun_2013]. 

## Understanding the Moran scatter plot
...
The `lisa` function calculates "local indicators of spatial association" [@anselin_1995]. LISA values are closely related to the Moran plot. If instead of centering `college`, we had converted it to z-scores by both centering and dividing by the standard deviation of `college`, the LISA value would be equal to the product of the z-scores and their spatial lag, $L_i = z_i * \tilde{z}_i$. The `lisa` function returns the LISA values and indicates which quadrant of the Moran plot the point is found:

```{r}
Li <- lisa(georgia$college, C, type = TRUE)
head(Li)
```
