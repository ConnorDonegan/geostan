...
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Measuring, visualizing, and simulating spatial autocorrelation}
...
moran_plot(y, W)
```
If instead, we use the binary coding scheme for the spatial weights matrix `C` then the y-axis (`C * y.c`) would show the *sum* of spatially-lagged values.
...
aple(y, W)
```
Again, for large number of observations, the APLE approximation will break down.
...
ggplot(ohio) +
  ...
  theme_void()
```
By default `geostan::sim_sar` will return a single n-length draw from the multivariate normal model (with covariance matrix specified following the SAR model). However, you can also use this function to generate multiple data sets at once. By setting the argument $m$ to an integer value greater than one, `sim_sar` will return an $m \times n$ matrix, with each row holding an n-length draw from the SAR model:
...
The **geostan** model returns samples drawn from the posterior probability distribution of $\mu$ (fitted values), which can be accessed using `fitted(fit)`. Here, $\hat{\mu}$ represents the mean of the posterior distribution of the parameter $\mu$, and is accessed using `fitted(fit)$mean`. Thus the following code reproduces the above results, but somewhat more transparently:
...
ggplot(ohio) +
  ...
  theme_void()
```
Another option for modeling continuous spatial outcomes is the conditional autoregressive (CAR) model (`geostan::stan_car`). Each of the spatial models in `geostan`---`stan_esf`, `stan_car`, and `stan_icar`---is returned with a spatial trend component that can be accessed using the `spatial` method.
