...
The middle panel represents spatial autocorrelation in the residuals as measured by the Moran coefficient (i.e., autocorrelation in the joint probability distribution of residuals), and the map shows the mean residual for each county (marginal posterior means). In this case, the MC histogram shows that there is only a small amount of residual autocorrelation, while the map indicates that this derives mainly from a north-south/metropolitan-rural trend. This indicates that we could improve this model by either adding one or more covariates (preferably something with direct substantive bearing on one's research question) or by using one of **geostan**'s spatial models. The trend in the residuals also helps us see that simply "shrinking" towards the mean mortality rate is undesirable in this case because we are well aware that county mortality rates are higher in the southern half of the state than in the greater Atlanta metropolitan area.
