...
get_shp(url, "nepal")
nepal <- st_read("nepal", "nepal")
```
We will reference per-capita income (PCI) `pcinc` and the percentage of people not expected to survive beyond age 40 `lif40`. To work with a fixed example, consider a model that hypothesizes premature mortality rates to be a linear function of per-capita income
\begin{equation}
 \text{early_mortality}_i = \alpha + \beta * \text{income}_i + e_i
 \end{equation}
...
fit <- stan_glm(lif40 ~ pcinc, data = nepal, prior_only = TRUE, refresh = 0)
```
...
```
By setting `prior_only = TRUE` we tell Stan to sample from the prior distributions only, ignoring the likelihood of the data. The default for `stan_glm` is to draw 4,000 samples (after throwing away the first 4,000 samples, used to identify the target distribution). We can visualize samples from the prior distributions:
...
```{r}
fit.c <- stan_glm(lif40 ~ pcinc, data = nepal, centerx = TRUE, refresh = 0)
```
...
\begin{equation}
  \text{early_mortality}_i = \alpha + EV_i * \beta_{EV} + \text{income}_i * \beta_{inc} + e_i
  \end{equation}
Each of the eigenvectors represent patterns of potential spatial autocorrelation. The challenge of ESF models is that they can have a very high number of parameters to estimate (one for each eigenvector) and often only a small number of them are far from zero. Thus we have some prior information on about how many coefficients are non-zero but we do not have prior information on *which* of them to select.
The `geostan` package addresses this by assigning a regularized horseshoe (RHS) prior model (@piironen_2017) to the coefficients `beta_ev`. The RHS model concentrates prior probability on zero, aggressively shrinking all of the coefficients to zero; but it also has heavy tails that open up where there is sufficient evidence of a non-zero coefficient, allowing a few parameters to escape the pull toward zero.

Again `geostan` will print its default priors before sampling from the model:

```{r}
C <- shape2mat(nepal)
fit.esf <- stan_esf(lif40 ~ pcinc, data = nepal, C = C, centerx = TRUE, prior_only = TRUE, refresh = 0)
```
...
The scale parameter $\sigma$ is assigned a half-Student's $t$ prior, which is truncated at zero (i.e. only positive values will be assigned non-zero probability). The Student's $t$ distribution has three parameters: degrees of freedom, location, and scale. So if wish to set the prior to Student's $t$ with 10 degrees of freedom, location 0, and scale 2, we would provide the following:
```{r eval = FALSE}
...
All of the models have the option to include a varying intercept term. These are similar to adding an indicator or dummy variable for each region, for instance, or any other geographic scale. However, they use hierarchical modeling to partially pool information across observations (often called 'random effects' terms, which is silly and misleading). For count data especially, this offers a way to evaluate rates of occurence while remaining attentive to the instability of rates when denominators are small. Generally, these models assign low probability to 'extreme' values, but what is considered 'extreme' or implausible is defined by a Gaussian distribution with parameters that are inferred from the data. The varying intercepts $\alpha_{[i]}$ are deviations from the global intercept, and their prior model is equivalent to:
$$
\alpha_{[i]} \sim Gauss(0, \tau_\alpha) \\
...
The default prior for the slab is `slab_df = 15` and `slab_scale = 0.5 * sd(y) / sd(EV[,1])`. The default prior for the global shrinkage parameter is described in @donegan_2020.
