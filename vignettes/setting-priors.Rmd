...
The new object `sf` is an `m`-by-`n` matrix, where `m` is the number of samples drawn from the posterior distribution and `n` is the number of observations in the data.
```{r}
dim(sf)
```
Thus the `i`th column contains samples from the marginal (prior) probability distribution for the spatial filter (i.e. spatially varying mean) at the location of the `i`th observation. This means one can summarize the probability distribution for the spatial filter at the `i`th observation by manipulating `sf` column-wise:
```{r}
# one at a time
mean(sf[,1])
...
apply(sf, 2, mean)
```
By contrast, each row is a draw from the *joint* probability distribution of the spatial filter. Each of the coefficients has the same marginal probability as `beta_ev[1]`. But samples from the joint distribution show that the horseshoe prior assigns high probability to the presence of positive spatial autocorrelation (without favoring any particular pattern). Each of the samples exhibit a different pattern, arising from various linear combinations of eigenvectors:
...
$$ \phi \sim MVGauss(0, \sigma^2 * (D - \alpha * C)^{-1}) $$

where $\phi$ is a vector of parameters to model and $\sigma$ is the scale parameter. C is a connectivity matrix while the diagonal elements of matrix D adjust the variance of each observation. For instance, the default specification sets the diagonal elements of D equal to the number of neighbors of each respective observation. $\alpha$ controls the degree of spatial autocorrelation. $\alpha$ will be less than zero when negative spatial autocorrelation patterns (similar values appear far from each other, like a checkerboard) predominate, and it is positive when the predominate pattern is positive spatial autocorrelation with a maximum value of 1. This parameter is not comparable to the spatial autocorrelation parameter from the SAR model or the APLE, despite some similarity.

The scale is given a half-Student's t prior ("half" meaning it is constrained to positive values only), requiring parameters for degrees of freedom, location, and scale. The following code sets the prior to 30 degrees of freedom, location 0, and unit scale:

```{r eval = FALSE}
fit <- stan_car(y ~ offset(log_population), data = data, C = C, prior_car_scale = c(30, 0, 1))
```
...
The following code chunk demonstrates how to calculate the `scale_factor` argument given a spatial connectivity matrix C, the `geostan::prep_icar_data` function, and a user-defined function `c_scale` (from @morris_2019):
