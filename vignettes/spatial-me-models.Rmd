...
```{r fig.width = 7}
sp_diag(georgia$ICE, georgia, name = "ICE", style = "B")
```
...
Because we have a joint probability distribution for $\delta_i$, we also have a probability distribution for any function of $\delta_i$. If we can calculate the Moran coefficient (MC, an index of spatial autocorrelation) for each sample from the posterior distribution of $\delta_i$, we get a probability distribution for the MC. The middle panel is a histogram of those MC values, with the average of the distribution printed at the top. The final panel is a map of the posterior mean for each $\delta_i$ value.

From the point-interval plot, we can see that a few of the counties with low ICE estimates have posterior distributions that have shifted slightly towards the mean. However, notice that the model still places substantial probability on values of the ICE that are *more* extreme than the raw ACS estimates. The MC histogram is centered on zero; that's acceptable but, ideally, this should be centered on a small negative value, indicating balanced spatial smoothing. 
...
```{r}
georgia[c(91, 105, 90, 39, 143), c("NAME", "population", "white", "black", "hisp", "ai", "ICE", "ICE.se", "college", "college.se")]
```

We can see that these areas are low population with low income, few college grads, and fairly high percent Black populations; they also have large standard errors on their ICE estimates. The somewhat large $\delta_i$ values for these counties are a result of the combination of unreliable estimates (large standard errors) while also being local outliers. 
...
If the `ME` list doesn't have a slot with `car_parts`, **geostan** will automatically use a non-spatial Student's t model instead of the CAR model. We can see how this results in more obviously patterned $\hat{\delta_i}$:

```{r fig.width = 7}
ME$car_parts <- NULL
fit_nsp <- stan_glm(log(rate.male) ~ ICE, data = georgia, ME = ME, prior_only = TRUE)
me_diag(fit_nsp, 'ICE', georgia)
```
