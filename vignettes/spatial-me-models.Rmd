...
The equation above states that the unknown values $\boldsymbol x$ are spatially autocorrelated, with a mean value of $\mu$ and conditional autoregressive (CAR) covariance matrix $$\boldsymbol \Sigma = (I - \rho C)^{-1} M,$$
...
We create a binary spatial connectivity matrix, and then pass it to `prep_car_data` to prepare our list of data for the CAR model:
