...
The equation above states that the unknown values $\boldsymbol x$ are spatially autocorrelated, with a mean value of $\mu$ and covariance matrix $$\boldsymbol \Sigma = (I - \rho C)^{-1} M,$$
...
In the following section, methods for examining MCMC samples of the modeled covariate values $\boldsymbol x$ will be illustrated. Note that the process of collecting/storing MCMC samples for $\boldsymbol x$ can be computationally burdensome. This can be avoided by using the `slim` argument, as in:
...
Using `slim = TRUE` may be faster (particularly for larger data sets), but will prevent the collection of MCMC samples for quantities of interest such as fitted values, pointwise log-likelihood values (required for WAIC), and modeled covariates $\boldsymbol x$. This option may be useful for preliminary examination of the models (e.g., to compare results to a standard model), or it may be appropriate to use this specification if the ME models have been evaluated in a previous run using `me_diag`, as discussed below. The `drop` argument to the model fitting functions (e.g., `stan_car`) provides more control over which parameters are saved.
